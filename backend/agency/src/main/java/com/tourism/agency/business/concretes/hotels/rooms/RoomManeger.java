...
public class RoomManeger implements RoomService {
	...
	public Result create(RoomCreateRequests createRequests) {
		...
		Room room = this.mapperService.forRequest().map(createRequests, Room.class);
		room.setId(-1);
	...
	public DataResult<List<RoomsGetAllResponses>> getAllReponses() {
		...
		List<RoomsGetAllResponses> getAllResponses = rooms.stream().map(room -> 
								this.mapperService.forResponse().map(room, RoomsGetAllResponses.class))
	...
	public Result update(RoomUpdateRequests updateRequests) {
		...
		Room room = this.mapperService.forRequest().map(updateRequests, Room.class);
		this.repository.save(room);	
